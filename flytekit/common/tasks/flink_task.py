...
class SdkGenericFlinkTask(_base_tasks.SdkTask):
    """
    This class includes the additional logic for building a task that executes as a Flink Job.
    ...
    def __init__(
        ...
        flink_job = _task_models.FlinkJob(
            flink_properties=flink_properties, jar_file=jar_file, main_class=main_class, args=args
        ).to_flyte_idl()
        ...
        for k, v in _six.iteritems(inputs):
            ...
            if sdk_type not in input_types_supported:
                raise _user_exceptions.FlyteValidationException(
                    "Input Type '{}' not supported.  Only Primitives are supported for Scala/Java Flink.".format(
                        sdk_type
                    )
                )
        super(SdkGenericFlinkTask, self)._validate_inputs(inputs)
